...
async function main() {
  ...
  const requests = [
    ...
    ...manualColumns.map(({ header, index }) => ({ repeatCell: { range: { sheetId, startRowIndex: 3, endRowIndex: 200, startColumnIndex: index, endColumnIndex: index + 1 }, cell: { userEnteredFormat: { backgroundColor: BLACK, textFormat: { foregroundColor: WHITE }, wrapStrategy: "WRAP" }, note: `BLACK = USER INPUT. Enter ${header.replaceAll("_", " ")} here; never edit the backend workbook.` }, fields: "userEnteredFormat,note" } })),
