...
export async function syncFreshDashboardInputs() {
  ...
  const tabs = ["UI_Occupancy", "UI_FONO_Supply", "UI_Shrampark_Supply", "UI_Enterprise_Demand", "UI_Enterprise_Supply", "UI_Finance", "UI_Collections", "UI_People", "UI_Actions", "UI_Approvals", "UI_Evidence", "UI_Targets"];
  ...
  const supplyTabs = ["UI_FONO_Supply", "UI_Shrampark_Supply", "UI_Enterprise_Supply"];
  const supply = supplyTabs.flatMap((name) => map(name, (row, table) => {
    ...
    return { "living hourly id": cell(table, row, "Record_ID"), "theatre id": cell(table, row, "Theatre_ID"), "studio id": cell(table, row, "Studio_ID") || cell(table, row, "Supply_Option_ID"), "supply model": name === "UI_FONO_Supply" ? "FONO" : name === "UI_Shrampark_Supply" ? "SP" : cell(table, row, "Supply_Model") || "ENTERPRISE", "contracted nests": available, "activation ready nests": ready, "occupied nests": 0, "occupancy ratio": 0, ...common(row, table) };
  }));
  ...
  const memberAdds = supplyTabs.flatMap((name) => map(name, (row, table) => {
    ...
    return { "demand id": `MEMBER-ADDS-${recordId}`, "enterprise id": name, "enterprise name": name === "UI_FONO_Supply" ? "FONO" : name === "UI_Shrampark_Supply" ? "Shrampark" : "Enterprise", "plant id": cell(table, row, "Studio_ID", "Supply_Option_ID"), "plant name": cell(table, row, "Studio_Name", "Factory_Name"), "role required": "Member Adds", "headcount required": required, "headcount matched": num(cell(table, row, "Member_Adds")), "activation required at": iso(cell(table, row, "Reporting_Date")), certainty: "Live input", status: cell(table, row, "Status") || "Live", "owner actor id": cell(table, row, "Business_Owner"), "opened at": iso(cell(table, row, "Reporting_Date")), ...common(row, table) };
  }));
