...
export function buildLiveEnterpriseDemandLoopPreview(rows: readonly EnterpriseSheetRow[], asOf: string): EnterpriseDemandLoopPreview | null {
  const inputs: EnterpriseContractInput[] = rows.flatMap((row) => {
    ...
    if (!demandId || !enterpriseName || !plantName || !arrivalAt || !Number.isFinite(Date.parse(arrivalAt)) || required <= 0 || !/compaign|campaign|lead|interest|under discussion|confirmed|open|contract|signed|won|onboard|live|closed|dropped/i.test(`${status} ${demandStatus}`)) return []
