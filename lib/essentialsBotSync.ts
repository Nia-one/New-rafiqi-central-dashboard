...
export async function syncEssentialsBotData() {
  ...
  const hourly = [...reportGroups.entries()].map(([key, g]) => {
    ...
    // Rafiqi_Order_Item_Costs V/W are the governed Essentials reporting
    // columns: V = unique buyers and W = buying value. Orders remain an
    // operational reconciliation source and must not override these totals.
