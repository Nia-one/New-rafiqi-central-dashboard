 ...
 */
export function AllocationContextStrip({ mismatchId }: { mismatchId?: string }) {
  ...
  return (
    <aside className="allocation-context" aria-label="Issue selected from the Overview page">
      <p className="allocation-context-tag">SELECTED ISSUE · SAMPLE DATA</p>
      <div className="allocation-context-body">
        <div><span>ISSUE</span><strong>{mismatch.label}</strong><small>{mismatch.theatre} · {mismatch.where}</small></div>
