...
function EnterpriseRecoverySummary({ preview, nextStep, actionableSteps, completedStages }: { preview: EnterpriseDemandLoopPreview; nextStep: JourneyStep | null; actionableSteps: number; completedStages: number }) {
  ...
  const routeCandidate = (nextStep?.candidateName ?? "No governed supply candidate").replace(/\bFONO\s+/i, "").replace(/\breserve\b/i, "Reserve")
  const pendingDemand = /under discussion|confirmed|open/i.test(preview.activeNode.demandStatus)
  ...
  return <section className="enterprise-recovery-summary" aria-labelledby="enterprise-recovery-title">
    ...
    <div className="enterprise-overview-charts">
      <figure className="enterprise-overview-chart is-capacity"><figcaption>{pendingDemand ? "Demand" : "Capacity"}</figcaption><div className="ds-composition enterprise-capacity-composition" role="img" aria-label={pendingDemand ? `Demand: ${required} Nests required, ${verified} matched and verified, ${gap} open.` : `Capacity: ${required} Nests required, ${verified} verified and clear, ${gap} vacant and require action.`} style={capacityStyle}><span className="ds-composition-segment" data-state="verified"><strong>{verified}</strong><small>{pendingDemand ? "Matched" : "Filled"}</small></span><span className="ds-composition-segment" data-state="unresolved"><strong>{gap}</strong><small>{pendingDemand ? "Open" : "Vacant"}</small></span></div><div className="enterprise-chart-scale" aria-hidden><span>0</span><b>{required}</b></div></figure>
    ...
    <div className="enterprise-recovery-route" role="note" aria-label={nextStep ? `${nextStep.ownerActorId} must confirm ${reserve} Nests from ${routeCandidate} now. ${open} Nests will remain open.` : `${preview.activeNode.ownerActorId} owns ${open} open demand Nests; no governed supply candidate is recorded.`}><strong>{nextStep?.ownerActorId ?? preview.activeNode.ownerActorId}</strong><i aria-hidden /><strong>{routeCandidate}</strong><i aria-hidden /><b>{reserve}</b><span>Nests</span><time dateTime={nextStep?.dueAt}>{nextStep ? "Now" : "Awaiting supply"}</time></div>
  </section>
}
...
export function EnterpriseDemandWorkspace({ preview }: Props) {
  ...
  return <div className="enterprise-demand-loop enterprise-form-workspace" aria-label="Enterprise Demand workspace">
    <ContextStrip label="Enterprise Demand plan context" items={[
      ...
      { label: "Plan state", value: pendingDemand ? preview.activeNode.demandStatus : lapsed ? "Lapsed · recovery required" : "Active", tone: lapsed && !pendingDemand ? "critical" : "neutral" },
      { label: "Scope", value: `${preview.activeNode.committedNests} confirmed demand Nests` },
    ]} />
    ...
    <section className="enterprise-secondary" aria-label="Supporting Enterprise Demand detail">
      ...
      <div className="enterprise-secondary-panel" role="tabpanel" hidden={secondaryView !== "controls"}>
        ...
        <section><h2>Demand-specific readiness</h2><dl><div><dt>Enterprise / plant</dt><dd>{preview.activeNode.enterpriseName} · {preview.activeNode.plantName}</dd></div><div><dt>Demand record</dt><dd>{preview.activeNode.contractId}</dd></div><div><dt>Status</dt><dd>{preview.activeNode.demandStatus}</dd></div><div><dt>Services</dt><dd>{preview.activeNode.contractedServices.join(", ") || "No additional services recorded"}</dd></div><div><dt>Plant reference</dt><dd>{preview.activeNode.plantReference}</dd></div><div><dt>Required by</dt><dd>{date(preview.activeNode.arrivalAt)}</dd></div></dl></section>
