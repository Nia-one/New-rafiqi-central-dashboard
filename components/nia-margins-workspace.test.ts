...
test("loop health derives live feed freshness, clocks, and verification state", () => {
  ...
  assert.match(source, /uniqueRowsById/)
  assert.match(source, /uniqueEvidenceRowsById/)
  assert.match(source, /statusRank/)
  assert.match(source, /\["verified", "approved", "accepted"\]/)
  assert.match(source, /label: "Living occupancy"[\s\S]*cadence: 1_440/)
  assert.match(source, /verifiedMarginOutcomes/)
...
test("headline measures use live finance and occupancy fields without inventing pillar CM2", () => {
  ...
  assert.match(source, /Enter each cumulative pillar total once in Finance_Daily/)
