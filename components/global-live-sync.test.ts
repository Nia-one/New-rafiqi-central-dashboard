...
test("global live sync runs one throttled live bot batch every minute", () => {
  ...
  assert.doesNotMatch(source, /\/api\/ops-data\?fresh=1/)
  assert.doesNotMatch(source, /attempt < 3/)
  assert.match(source, /\/api\/ops-data\?live=1/)
  assert.match(source, /report\.changedRows/)
  assert.match(source, /CONVERGENCE_REFRESH_MS = \[0, 20_000\]/)
