...
export function LivingScreen({ focus, allocationFocus, liveOpsData }: { focus?: LivingSection; allocationFocus?: string; liveOpsData: any }) {
  ...
  const liveFonoOccupancyRows = (liveOpsData.fonoOccupancy ?? []).map((row:any) => [
    String(row.studio ?? ""),
    String(row.theatre ?? ""),
    String(row.available ?? 0),
    String(row.occupied ?? 0),
    `${Math.round((row.occupancy ?? 0) * 100)}%`,
    "",
    ""
  ])
  ...
  return <DashboardSectionAccordion className="pillar-screen living-screen" ariaLabel="Living sections" sections={[
    ...
    <section id="fono" className={`operating-section ${focus === "fono" ? "focused-section" : ""}`}>
      ...
      <div className="channel-grid fono-channel-grid">
        ...
        <DataTable className="compact-table occupancy-table" caption="FONO Studio occupancy" columns={["STUDIO","THEATRE","AVAILABLE","OCCUPIED","OCCUPANCY","DAYS LIVE","THEATRE OPS OWNER"]} rows={liveFonoOccupancyRows} />
      </div>
    </section>
