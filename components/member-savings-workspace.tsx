  ...
  return <DashboardSectionAccordion className={styles.workspace} ariaLabel="Member Savings sections" sections={[
    ...
    <details className={styles.auditDetails}>
      ...
      <div className={styles.auditBody}>
        ...
        <section><strong>Weekly savings-message inputs</strong>{isLive ? (liveServices.length ? liveServices.map((service) => <dl key={service.serviceId}><div><dt>Service</dt><dd>{service.serviceName} · {service.studio}</dd></div><div><dt>Recorded saving</dt><dd>₹{service.memberSavingsInr.toLocaleString("en-IN")}</dd></div><div><dt>Freshness</dt><dd>{liveFreshness?.staleFeedCount ? `${liveFreshness.staleFeedCount} connected feed stale` : "Current connected snapshot"}</dd></div><div><dt>Mode</dt><dd>Google Sheet · read-only · {weeklyMessageStatusByServiceId.get(service.serviceId)}</dd></div></dl>) : <p>No eligible Essentials_Hourly savings input is recorded.</p>) : preview.weeklyMessageInputs.map((input) => <dl key={input.serviceRef}><div><dt>Service</dt><dd>Protected service reference</dd></div><div><dt>Verified saving</dt><dd>{input.verifiedSavingsInr === null ? "Unavailable" : `₹${input.verifiedSavingsInr}`}</dd></div><div><dt>Freshness</dt><dd>{input.dataFreshness}</dd></div><div><dt>Mode</dt><dd>{input.mode} · sent {String(input.sent)}</dd></div></dl>)}</section>
