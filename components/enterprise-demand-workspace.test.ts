...
test("the first viewport leads with context then one lapsed-plan recovery action", () => {
  ...
  const decision = html.indexOf("enterprise-recovery-summary")
  ...
  assert.match(html, /Vikram Solar.*26 July/)
  assert.match(html, /Verified \/ clear/)
  assert.match(html, /Vacant \/ action/)
  assert.match(html, /FONO Supply JCO/)
  assert.match(html, /Oragadam Reserve A/)
  assert.match(html, />24<\/b><span>Nests<\/span>/)
  assert.match(html, />Now<\/time>/)
})

test("score rows carry units and identified nearby supply is not presented as verified coverage", () => {
  const html = renderWorkspace()
  assert.match(html, /Capacity: 180 Nests required, 140 verified and clear, 40 vacant and require action/)
  assert.match(html, /Arrival: 2 of 8 stages are clear; 6 require action/)
  assert.match(html, /Nearby: 40 Nests are identified but require action; 24 Nests are the current recovery candidate and remain unverified/)
...
test("today's work and assigned exceptions are visible with owner due progress and outcome", () => {
  const html = renderWorkspace()
  assert.match(html, /enterprise-work-board/)
  assert.match(html, /Work top to bottom/)
  assert.match(html, /Ordered Enterprise Demand work queue/)
  assert.match(html, /Record outcome/)
  assert.match(html, /After Ring 1/)
  assert.match(html, /Assigned support/)
  assert.match(html, />Owner</)
  assert.match(html, />Due</)
  assert.match(componentSource, /data-timing=\{later \? "later" : "now"\}/)
})
