...
import { formatInr, opsData, trajectory } from "@/lib/ops-data"
...
export function CmTrajectory() {
  ...
  return (
    <section className="story-section trajectory" aria-labelledby="trajectory-title">
      ...
      <ChartContainer config={chartConfig} className="cm-chart" aria-label="Cumulative contribution margin actual, target path and projected month end">
        <LineChart data={run.points} margin={{ top: 20, right: 28, left: 4, bottom: 4 }} accessibilityLayer>
          <CartesianGrid vertical={false} />
          <XAxis dataKey="day" tickLine={false} axisLine={false} tickFormatter={(value) => value === 1 || value === opsData.meta.day || value === 31 ? `Jul ${value}` : ""} />
      ...
      <div className="chart-labels" aria-hidden><span><i className="actual" />Actual through Jul {opsData.meta.day}</span><span><i className="projection" />Forecast</span><span><i className="target" />Target</span></div>
