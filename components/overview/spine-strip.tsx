...
import { formatInr, formatSpineValue, metricPace, metricVariance, opsData } from "@/lib/ops-data"

export function SpineStrip({ onNavigate }: { onNavigate: (route: DashboardRoute, mismatchId?: string) => void }) {
    ...
    <section className="story-section" aria-labelledby="spine-title">
      ...
      <div className="spine-scroll" role="group" aria-label="Operating spine; scroll horizontally on small screens">
        <div className="spine-strip">
          {opsData.spine.map((metric, index) => {
            ...
            return <div className="spine-unit" key={metric.id}>
              ...
              {index < opsData.spine.length - 1 && <button className={`spine-link ${index === 0 && top ? "broken" : ""}`} onClick={() => (index === 0 && top ? onNavigate(top.laneTarget, top.id) : onNavigate(OVERVIEW_ROUTES[opsData.spine[index + 1].id]))} aria-label={index === 0 && top ? `Highest-ranked mismatch: ${top.label}; open ${top.laneTarget.screen}${top.laneTarget.subsection ? ` ${top.laneTarget.subsection}` : ""}` : `Open ${OVERVIEW_ROUTES[opsData.spine[index + 1].id].screen}${OVERVIEW_ROUTES[opsData.spine[index + 1].id].subsection ? ` ${OVERVIEW_ROUTES[opsData.spine[index + 1].id].subsection}` : ""}; continue downstream`}><ArrowRight aria-hidden /></button>}
            </div>
          })}
        </div>
      </div>
