...
export function EnterpriseDemandSupplyScreen({ rows, embedded = false }: { rows: DemandSupplyMatch[]; embedded?: boolean }) {
  ...
  return <div className="enterprise-match-screen">
    ...
    <section className="business-report-panel">
      ...
      <div className="table-wrap"><table><thead><tr><th>THEATRE</th><th>ENTERPRISE / LOCATION</th><th>DEMAND STATUS</th><th>MATCH RANK</th><th>MATCHED PROPERTY</th><th>NEAREST CHECKED PROPERTY / LOCATION</th><th>HUNTED BY</th><th>AUTO ROUTE</th><th>GOOGLE MAPS CHECK</th><th>APPROVAL STATUS</th><th>CRITERIA</th></tr></thead><tbody>{filtered.map((row) => {
        const routeUrl = googleMapsUrl(row)
        const ready = isFounderReady(row)
        return <tr key={`${row.theatre}-${row.company}-${row.property}`}><td>{row.theatre}</td><td><strong>{row.company}</strong><small>{row.demandLocation || "Location not recorded"}</small></td><td>{row.demandStatus}</td><td>{row.rank ?? "—"}</td><td><strong>{row.eligible || view === "All evaluated properties" ? row.property : "No option within criteria"}</strong><small>{row.propertyOwner ? `Owner: ${row.propertyOwner}` : ""}</small></td><td>{!row.eligible && view !== "All evaluated properties" ? <><strong>{row.property}</strong><small>{row.propertyOwner ? `Owner: ${row.propertyOwner}` : "Nearest available supply"}</small></> : "—"}</td><td>{row.eligible || view === "All evaluated properties" ? row.hunter : "—"}</td><td><strong>{row.bikeDistanceKm.toFixed(2)} km</strong><small>{row.bikeMinutes.toFixed(1)} min · estimated</small></td><td>{routeUrl ? <a href={routeUrl} target="_blank" rel="noreferrer"><strong>Open Google Maps</strong></a> : "Coordinates unavailable"}<small>{row.verifiedDistanceKm !== undefined && row.verifiedBikeMinutes !== undefined ? `${row.verifiedDistanceKm.toFixed(2)} km · ${row.verifiedBikeMinutes.toFixed(1)} min` : "Enter verified values in result sheet"}</small></td><td><strong>{ready ? "Founder-ready" : row.verificationStatus || (row.eligible ? "Provisional match" : "Provisional no option")}</strong><small>{ready && row.verifiedBy ? `Verified by ${row.verifiedBy}` : "Manual verification required"}</small></td><td>{row.rule}</td></tr>
      })}</tbody></table></div>
