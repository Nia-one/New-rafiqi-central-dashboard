...
export function EnterpriseDemandSupplyScreen({ rows }: { rows: DemandSupplyMatch[] }) {
  ...
  return <div className="enterprise-match-screen">
    ...
    <section className="business-report-panel">
      <div className="section-heading"><div><p className="pillar-kicker">THEATRE-WISE MATCHING</p><h2>Closest eligible property, ranked by Google Maps bike time and distance</h2></div><label>Theatre <select value={theatre} onChange={(event) => setTheatre(event.target.value)}>{theatres.map((item) => <option key={item}>{item}</option>)}</select></label></div>
      ...
      <p className="footer-note">Distance and duration come from Google Maps Routes API in TWO_WHEELER mode; no straight-line estimate is used. Coromandel rule: maximum 15 km and 30 bike minutes. Deccan rule: maximum 10 km when its source is connected. Two-wheeler routes are a Google beta feature and may not always contain the optimal motorcycle path.</p>
    </section>
  </div>
}
