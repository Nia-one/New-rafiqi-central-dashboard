...
function CollectionsControl({ rows }: { rows: readonly Row[] }) {
  ...
  const collected = collections.reduce((sum, row) => sum + amount(row, "total collected inr"), 0)
  const due = collections.reduce((sum, row) => sum + Math.max(0, amount(row, "current due inr") || amount(row, "total billed inr") - amount(row, "total collected inr")), 0)
  ...
  const grouped = (key: string) => [...new Set(collections.map((row) => text(row, key) || "Unassigned"))].map((label) => {
    ...
    return [label, String(group.length), inr(group.reduce((sum, row) => sum + amount(row, "total billed inr"), 0)), inr(group.reduce((sum, row) => sum + amount(row, "total collected inr"), 0)), inr(group.reduce((sum, row) => sum + Math.max(0, amount(row, "current due inr") || amount(row, "total billed inr") - amount(row, "total collected inr")), 0))]
  ...
  return <>
    <section className="operating-section" aria-label="Collections control"><h2>Collections control</h2><div className="people-headline" data-kpi-group>
      ...
      <article><span>Collected</span><strong>{inr(collected)}</strong><small>Verified collection input</small></article>
      <article><span>Pending</span><strong>{inr(due)}</strong><small>{open.length} open records</small></article>
      <article><span>Overdue</span><strong>{overdue.length}</strong><small>Governed recovery required</small></article>
    </div></section>
    <section className="operating-section"><h2>Invoice-wise pending collection</h2><DataTable caption="Invoice-wise pending collection" columns={["INVOICE", "MEMBER / CUSTOMER", "STUDIO", "THEATRE", "BILLED", "COLLECTED", "PENDING", "DUE DATE", "STATUS"]} rows={collections.map((row) => [text(row, "invoice id") || text(row, "finance daily id"), text(row, "customer or member ref") || "No data", text(row, "studio name") || text(row, "studio id") || "No data", text(row, "theatre name") || text(row, "theatre id") || "No data", inr(amount(row, "total billed inr")), inr(amount(row, "total collected inr")), inr(Math.max(0, amount(row, "current due inr") || amount(row, "total billed inr") - amount(row, "total collected inr"))), text(row, "due date") || "No data", text(row, "reconciliation status") || "Open"])} /></section>
