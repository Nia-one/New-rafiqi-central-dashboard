...
import { buildEnterpriseDemandSupplyStatus } from "@/lib/live-mappers/enterprise-demand-supply"
...
function EssentialsAttachmentReport({ essentials }: { essentials: ReturnType<typeof buildBusinessReportData>["essentials"] }) {
  ...
  return <section className="business-report-panel essentials-report-card">
    <div className="essentials-report-heading">
      <p className="pillar-kicker"><b>6</b> ESSENTIALS</p>
...
export function BusinessReportScreen({ liveOpsData }: { liveOpsData: any }) {
  ...
  const demandSupply = buildEnterpriseDemandSupplyStatus(liveOpsData?.enterpriseDemand ?? [])
  const top = [
  ...
  return <DashboardSectionAccordion className="pillar-screen business-report-screen" ariaLabel="Business Report sections" sections={[
    ...
    { title: "Enterprise Demand vs Supply Status", summary: `${demandSupply.rows.length} company-property records · ${demandSupply.performance.length} Theatre × hunter groups` },
    ...
    <section className="business-report-panel"><p className="pillar-kicker">5 · ENTERPRISE DEMAND VS SUPPLY STATUS</p><h2>Every Enterprise demand is shown against its hunted FONO properties.</h2><p>Company and demand status come from Enterprise Demand. Property, location, hunter and property status come from the FONO Funnel and join through Enterprise Demand ID or Enterprise ID.</p><DataTable caption="Enterprise company to hunted property status" columns={["COMPANY", "THEATRE", "DEMAND STATUS", "PROPERTY", "LOCATION", "HUNTED BY", "PROPERTY STATUS", "MATCH STATUS"]} rows={demandSupply.rows.length ? demandSupply.rows.map((row) => [row.company, row.theatre, row.demandStatus, row.property, row.location, row.hunter, row.propertyStatus, row.matchStatus]) : [["No Enterprise demand/property mapping", "—", "—", "—", "—", "—", "—", "—"]]} /><div style={{ marginTop: 24 }}><DataTable caption="Property hunting performance by Theatre and person" columns={["THEATRE", "HUNTING PERSON", "PROPERTIES HUNTED", "MAPPED TO DEMAND", "SELECTED / CONTRACTED", "MAPPING RATE"]} rows={demandSupply.performance.length ? demandSupply.performance.map((row) => [row.theatre, row.hunter, String(row.hunted), String(row.mapped), String(row.contracted), row.hunted ? `${Math.round(row.mapped / row.hunted * 100)}%` : "0%"]) : [["No hunting activity", "—", "0", "0", "0", "0%"]]} /></div></section>
