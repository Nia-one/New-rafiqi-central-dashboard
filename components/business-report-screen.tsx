...
export function BusinessReportScreen({ liveOpsData }: { liveOpsData: any }) {
  ...
  return <DashboardSectionAccordion className="pillar-screen business-report-screen" ariaLabel="Business Report sections" sections={[
    ...
    <section className="business-report-panel"><p className="pillar-kicker">2 · CONTRIBUTION MARGIN</p><h2>{report.contribution.actual ? `${compactInr(report.contribution.actual)} explicitly recorded CM.` : "Governed contribution margin is not recorded."}</h2><DataTable caption="Live contribution margin inputs" columns={["STREAM", "RECORDED CM"]} rows={[["Work", compactInr(report.contribution.work)], ["Living / Finance", compactInr(report.contribution.living)], ["Essentials", compactInr(report.contribution.essentials)], ["Pipeline", report.contribution.pipelineRecorded ? compactInr(report.contribution.pipeline) : "Not recorded"], ["Total actuals", compactInr(report.contribution.actual)]]} /></section>
    ...
    <section className="business-report-panel"><p className="pillar-kicker">4 · ENTERPRISE DEMAND</p><h2>{report.enterprise.records} governed Enterprise records.</h2><div className="business-split"><Funnel title="Conversion funnel" values={Object.entries(report.enterprise.stages) as [string, number][]} /><DataTable caption="Enterprise records by Theatre" columns={["THEATRE", "RECORDS"]} rows={report.enterprise.byTheatre.map((row) => [row.theatre, String(row.records)])} /></div></section>
