...
export function BusinessReportScreen({ liveOpsData }: { liveOpsData: any }) {
  ...
  return <DashboardSectionAccordion className="pillar-screen business-report-screen" ariaLabel="Business Report sections" sections={[
    ...
    <section className="business-report-panel"><p className="pillar-kicker">4 · ENTERPRISE DEMAND &amp; SUPPLY</p><h2>{report.enterprise.records} demand records · {report.enterprise.supplyRecords} supply records.</h2><div className="business-split"><Funnel title="Demand conversion funnel" values={Object.entries(report.enterprise.stages) as [string, number][]} /><Funnel title="Supply conversion funnel" values={Object.entries(report.enterprise.supplyStages) as [string, number][]} /></div><DataTable caption="Enterprise demand records by Theatre" columns={["THEATRE", "RECORDS"]} rows={report.enterprise.byTheatre.map((row) => [row.theatre, String(row.records)])} /></section>
