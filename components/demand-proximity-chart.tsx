...
function positionOption(distanceKm: number, angle: number) {
  ...
  return {
    left: `${CENTER + Math.cos(radians) * radius}px`,
    top: `${CENTER + Math.sin(radians) * radius}px`,
  }
}
...
export function DemandProximityChart({ node }: { node: DemandProximityNode }) {
  ...
  return <article className="proximity-card" aria-labelledby={`${node.id}-title`}>
    <header>
      <div>
        <p className="pillar-kicker">OPEN DEMAND</p>
        <h3 id={`${node.id}-title`}>{node.demandName}</h3>
        <p>{node.members.toLocaleString("en-IN")} Members · activate {node.activation}</p>
      </div>
      <span className="proximity-option-count">{node.options.length} options</span>
    </header>

    <div className="proximity-plot" aria-hidden="true">
      {[5, 3, 2].map(distance => <div className={`proximity-ring ring-${distance}`} key={distance}><span>{distance} km</span></div>)}
      <div className="demand-node"><span>Demand</span><strong>{node.shortName}</strong><small>{node.members} Members</small></div>
      {node.options.map(option => <div className={`supply-option ${option.distanceKm <= 2 ? "is-viable" : ""}`} key={option.id} style={positionOption(option.distanceKm, option.angle)}>
        <i />
        <span><strong>{option.name}</strong><small>{option.distanceKm.toFixed(1)} km</small></span>
      </div>)}
    </div>
