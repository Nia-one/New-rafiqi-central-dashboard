...
export function CashControlWorkspace({ preview }: Props) {
  ...
  return <DashboardSectionAccordion className={styles.workspace} ariaLabel="Cash and Control sections" sections={[
    ...
    { title: "Loop health", summary: `${preview.loopHealth.state} · ${preview.loopHealth.verification.verified}/${preview.loopHealth.verification.claimed} verified`, status: preview.loopHealth.state === "Confirmed" ? "good" : "warn" },
    ...
    { title: "Target to result", summary: `${preview.summary.current} current · ${preview.summary.gap} gap`, status: typeof preview.summary.gap === "number" && preview.summary.gap === 0 ? "good" : "warn" },
    { title: "Headline measures", summary: `${preview.measures.length} financial controls at a glance`, lens: "decide" },
    { title: "Control implication", summary: "Cash is protected; destination approval still blocks the cascade.", lens: "decide" },
    { title: "Monthly control path", summary: `${preview.closureCounts.verified}/${preview.closureCounts.claimed} closures verified`, status: preview.closureCounts.verified >= preview.closureCounts.claimed ? "good" : "warn", lens: "decide" },
    { title: "Control path implication", summary: "The approved target cannot fall silently.", lens: "decide" },
    { title: "Channel recommendation", summary: `${preview.channelRecommendations.length} evidence-ranked options · recommendation only`, lens: "decide" },
    { title: "Channel implication", summary: "No allocation is imposed before approval.", lens: "decide" },
    { title: "Open work", summary: `${tasks.length} owned command tasks remain open`, status: tasks.length > 0 ? "warn" : "good", lens: "operate" },
    { title: "Human approvals", summary: `${preview.approvals.length} financial decisions require named authority`, status: preview.approvals.length > 0 ? "warn" : "good" },
    { title: "Background record", summary: `${audit.length} local shadow events · governed controls retained`, lens: "operate" },
    { title: "Decision required", summary: `Owner ${preview.summary.owner} · due ${decisionDue}`, status: "bad" },
    { title: "Recovery rule", summary: "Missed work rolls forward; claimed activity never closes without evidence." },
    { title: "Source and confidence", summary: `${preview.source.name} · Production confidence Low`, status: "warn" },
  ]}>
    ...
    <section className={styles.workPanel} aria-label="Open Cash and Control work">
      ...
      <OperationalCardStack label="Cash and Control command work">{tasks.map((task) => <OperationalCard key={task.actionId} title={task.issue} domain={task.actionId} status={task.state} fields={[{ label: "Owner", value: task.owner }, { label: "Due", value: <time dateTime={task.dueAt}>{date(task.dueAt)}</time> }, { label: "Progress", value: task.progress }, { label: "Expected verified result", value: task.expectedVerifiedResult }, { label: "Verified result", value: task.verifiedResult }]}><LensGate show="operate"><div className={styles.shadowControl}><TokenSelect ariaLabel={`Shadow outcome for ${task.issue}`} value={selected[task.actionId] ?? "Unresolved"} options={["Unresolved", "Evidence received", "Failed evidence", "Human approval required", "Missed hour"] as const} onChange={(outcome) => setSelected((current) => ({ ...current, [task.actionId]: outcome }))} /><button type="button" onClick={() => recordShadowOutcome(task)}>Record locally</button><small>No approval, payment, message or Production write</small></div></LensGate></OperationalCard>)}</OperationalCardStack>
    </section>
