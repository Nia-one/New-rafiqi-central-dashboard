...
export function ControlTower({ liveOpsData, enterpriseDemandPreview, controlledAutonomyPreview, niaMarginsPreview, newAddsPreview, memberEngagementPreview, memberSavingsPreview, niaGrowthPreview, onOpenWorkspace }: ControlTowerProps) {
  ...
  const enterpriseVerifiedMatches = enterpriseMatchRows.filter((row) => row.rank === 1 && String(row.verificationStatus ?? "").trim().toLowerCase() === "verified match" && String(row.verifiedProperty ?? "").trim().toLowerCase() === String(row.property ?? "").trim().toLowerCase() && Number.isFinite(row.verifiedDistanceKm) && Number.isFinite(row.verifiedBikeMinutes)).length
  ...
  const loops = [
    enterpriseRows.length
      ? { name: "Enterprise demand", current: `${enterpriseApproved} active leads · ${enterpriseMatchedProperties} provisional · ${enterpriseVerifiedMatches} verified`, target: `${enterpriseRows.length} sheet rows`, gap: `${enterpriseExcluded} excluded · ${enterpriseNoOption} no option`, due: enterpriseDue ? displayDate(enterpriseDue) : "Stage review", verified: enterpriseUpdated ? displayDate(enterpriseUpdated) : "Backend sheet" }
