...
export function canonicalMemberAddsControl(summary: NewAddsPreview["taskSummary"]) {
  return Object.freeze({
    current: String(summary.current),
    target: String(summary.target),
    gap: String(summary.gap),
    owner: String(summary.owner || "Unassigned"),
    rootCause: `${String(summary.current)} verified Member Adds are recorded against the ${String(summary.target)} governed target, leaving ${String(summary.gap)} still open.`,
  })
}
...
export function ControlTower({ liveOpsData, enterpriseDemandPreview, controlledAutonomyPreview, niaMarginsPreview, newAddsPreview, memberEngagementPreview, memberSavingsPreview, niaGrowthPreview, onOpenWorkspace }: ControlTowerProps) {
  ...
  const ownerRegistry = (liveOpsData?.ownerRegistry ?? []) as Record<string, unknown>[]
  const registeredOwner = (...needles: string[]) => {
    const normalized = needles.map((needle) => needle.toLowerCase())
    const match = ownerRegistry.find((row) => {
      const identity = ["assignment id", "vertical", "scope", "business responsibility"].map((field) => sourceValue(row, field)).join(" ").toLowerCase()
      return sourceValue(row, "status").toLowerCase() === "active"
        && sourceValue(row, "role type").toLowerCase().includes("owner")
        && normalized.every((needle) => identity.includes(needle))
    })
    return sourceValue(match, "owner name")
  }
  const resolvedOwner = (candidate: unknown, fallback: string) => {
    const owner = String(candidate ?? "").trim()
  ...
  const enterpriseMatchRows = ((liveOpsData?.enterpriseDemandSupplyMatches ?? []) as { theatre?: string; company?: string; property?: string; rank?: number | null; dataIssue?: string; verifiedProperty?: string; verifiedDistanceKm?: number; verifiedBikeMinutes?: number; verificationStatus?: string }[]).filter((row) => !row.dataIssue)
  const enterpriseMatchedProperties = enterpriseMatchRows.filter((row) => row.rank === 1).length
  const enterpriseFounderReady = enterpriseMatchRows.filter((row) => row.rank === 1 && String(row.verificationStatus ?? "").trim().toLowerCase() === "verified match" && String(row.verifiedProperty ?? "").trim().toLowerCase() === String(row.property ?? "").trim().toLowerCase() && Number.isFinite(row.verifiedDistanceKm) && Number.isFinite(row.verifiedBikeMinutes)).length
  ...
  const enterpriseRequired = enterpriseRows.length
  const enterpriseMatched = enterpriseApproved
  ...
  const loops = [
    enterpriseRows.length
      ? { name: "Enterprise demand", current: `${enterpriseApproved} active leads · ${enterpriseMatchedProperties} provisional · ${enterpriseFounderReady} Founder-ready`, target: `${enterpriseRows.length} sheet rows`, gap: `${enterpriseExcluded} excluded · ${enterpriseNoOption} no option`, due: enterpriseDue ? displayDate(enterpriseDue) : "Stage review", verified: enterpriseUpdated ? displayDate(enterpriseUpdated) : "Backend sheet" }
  ...
  const enterpriseStages: readonly (readonly [number, string, string, number])[] = ENTERPRISE_PIPELINE_STAGES.map((stage) => [enterpriseStageCounts[stage], stage, "Live sheet lead records", 0] as const)
  const memberStages: readonly (readonly [number, string, string, number])[] = [[Number(livePreviews.newAddsPreview.taskSummary.target) || 0, "Target fills", "Governed target", 0], [Number(livePreviews.newAddsPreview.taskSummary.current) || 0, "Actual Member Adds", "Source-reported actuals", Number(livePreviews.newAddsPreview.taskSummary.gap) || 0]]
  const viewFor = (consoleName: string) => {
    ...
    const isMemberAddsControl = consoleName === "Member Adds"
    const canonicalMemberAdds = isMemberAddsControl ? canonicalMemberAddsControl(newAddsPreview.taskSummary) : null
      ...
      <div className="tower-scroll-plane">
        <div className="tower-content-row">
          <div className="tower-canvas">
            ...
            {activeConsole === "All consoles" || activeConsole === "Enterprise Demand" ? <Funnel title="Enterprise Demand stage-wise lead count" weak="Lead" stages={enterpriseStages} source="Enterprise_Demand live sheet rows" /> : null}
